...
const { ensurePrivateDirectory } = require("./store");
...
async function initializeSession(config, provider) {
  ...
  process.stdout.write([
    "",
    "========================================",
    ` ${provider.name} Session Initialization`,
    "========================================",
    "",
    `Profile: ${config.PROFILE_DIR}`,
    "",
    `1. Sign in to ${provider.name} in the opened Chrome window.`,
    "2. Complete any required verification challenges.",
    "3. Confirm the conversation page is usable.",
...
module.exports = { initializeSession, validateChromePath };
